...
mutable struct Results
    n_func::Array{Float64, 5} # Optimal Workers
    k_func::Array{Float64, 5} # Optimal Capital
    ex_func::Array{Float64, 5} # Optimal Export Decision
    ...
    val_func::Array{Float64, 5} # Value function
    τ::Float64
    ...
    FC_0_min::Float64 # FC_0 Lower Bound
    FC_0_max::Float64 # FC_0 Upper Bound
    nFC_0::Int64 #Number of FC_0 states 
    FC_0_grid::Array{Float64,1} # FC_0 Grid
    tauchen_trans_FC_0::Array{Float64,2} # Tauchen's Method Transition Probs for FC_0
    FC_1::Float64 # Fixed Cost reentry based on Last Export
    σ_FC_1::Float64 # i.i.d. SD Normal Shock to FC_1
    FC_1_min::Float64 # FC_0 Lower Bound
    FC_1_max::Float64 # FC_0 Upper Bound
    nFC_1::Int64 #Number of FC_0 states 
    FC_1_grid::Array{Float64,1} # FC_0 Grid
    tauchen_trans_FC_1::Array{Float64,2} # Tauchen's Method Transition Probs for FC_1
end
...
function Initialize()
    ...
    σ_FC_0 = 0.02 # Random variation in FC_0
    FC_0_min = FC_0-3*σ_FC_0
    FC_0_max = FC_0+3*σ_FC_0
    nFC_0 = 5
    FC_0_grid = collect(range(FC_0_min, length = nFC_0, stop = FC_0_max))
    # Tauchen for FC_0, no persistence, IID around FC_0 w/σ_FC_0
    tauchen_res_FC_0 = tauchen(nFC_0, 0.0, σ_FC_0, FC_0)
    tauchen_trans_FC_0 = tauchen_res_FC_0.p

   # FC_1 Definitions
    FC_1 = 0.5038568304392312 
    σ_FC_1 = 0.02 # Random variation in FC_1
    FC_1_min = FC_1-3*σ_FC_1
    FC_1_max = FC_1+3*σ_FC_1
    nFC_1 = 5
    FC_1_grid = collect(range(FC_1_min, length = nFC_1, stop = FC_1_max))
    # Tauchen for FC_1, no persistence, IID around FC_1 w/σ_FC_1
    tauchen_res_FC_1 = tauchen(nFC_1, 0.0, σ_FC_1, FC_1)
    tauchen_trans_FC_1 = tauchen_res_FC_1.p

    n_func = Array{Float64}(zeros(prim.nQ, prim.nϵ, n_prev_ex, nFC_0, nFC_1)) # initial worker function guess
    k_func = Array{Float64}(zeros(prim.nQ, prim.nϵ, n_prev_ex, nFC_0, nFC_1)) # initial capital function guess
    ex_func = Array{Float64}(zeros(prim.nQ, prim.nϵ, n_prev_ex, nFC_0, nFC_1)) # initial export function guess
    val_func = Array{Float64}(zeros(prim.nQ, prim.nϵ, n_prev_ex, nFC_0, nFC_1)) # initial value function guess

    res = Results(n_func, k_func, ex_func, ex_cap, ϵ, Q, ϵ_experiment, Q_experiment, val_func, τ, C_star, σ_e, ρ_e, 
                  n_prev_ex, prev_ex_grid, tauchen_trans_Q, tauchen_trans_e, FC_0, σ_FC_0, FC_0_min, FC_0_max, nFC_0, FC_0_grid, tauchen_trans_FC_0,
                  FC_1, σ_FC_1, FC_1_min, FC_1_max, nFC_1, FC_1_grid, tauchen_trans_FC_1) #initialize results struct
    prim, res #return deliverables
end

#Bellman Operator
function Bellman(prim::Primitives,res::Results)
    @unpack val_func, ex_func, n_func, k_func, τ, C_star, σ_e, ρ_e, prev_ex_grid, n_prev_ex, tauchen_trans_Q, tauchen_trans_e = res #unpack value function
    @unpack nFC_1, FC_1_grid, tauchen_trans_FC_1 = res # Unpack FC_1 Variables
    @unpack nFC_0, FC_0_grid, tauchen_trans_FC_0 = res # Unpack FC_0 Variables
    @unpack Q_grid, Q_max, ϵ_grid, ϵ_max, R, r, w, σ_q, nQ, Q_grid, nϵ, ϵ_grid, ρ_q, α_n, θ, C, γ_0, γ_1 = prim #unpack model primitives
    v_next = zeros(nQ,nϵ,n_prev_ex,nFC_0,nFC_1) #next guess of value function to fill
    ...
    for Q_index = 1:nQ   
        ...
        for ϵ_index = 1:nϵ
            ...
            for prev_ex_index in 1:n_prev_ex
                ...
                for FC_0_index = 1:nFC_0
                    rand_FC_0 = FC_0_grid[FC_0_index]

                    for FC_1_index = 1:nFC_1
                        rand_FC_1 = FC_1_grid[FC_1_index]
                        ...
                        for ex_decision in 0:1 #loop over possible export decision
                            Π(x) = -1*((1+ex_decision*(Q*(1-τ))^θ*C_star/C)^(1/θ)*C^(1/θ)*ϵ*x[1]^(α_n*(θ-1)/θ)*x[2]^((1-α_n)*(θ-1)/θ)-w*x[1]-r*x[2])
                            x0 = [0.1, 0.1]
                            lower = [0.01, 0.01]
                            upper = [Inf, Inf]
                            ...
                            for ϵp_index in 1:nϵ
                                for Qp_index in 1:nQ
                                    for FC_0p_index in 1:nFC_0
                                        for FC_1p_index in 1:nFC_1
                                            continuation_val += R*tauchen_trans_e[ϵ_index,ϵp_index]*tauchen_trans_Q[Q_index,Qp_index]*tauchen_trans_FC_0[FC_0_index,FC_0p_index]*tauchen_trans_FC_1[FC_1_index,FC_1p_index]*val_func[Qp_index,ϵp_index,prev_ex_index,FC_0p_index,FC_1p_index]
                                        end
                                    end
                                end
                            end
                            ...
                            if ex_decision == 0
                                ...
                                if prev_ex_index == n_prev_ex
                                    ...
                                    total_Π += -1*((1-prev_ex)*rand_FC_0+rand_FC_1)
                                end

                                if total_Π > Π_prev
                                    ex_func[Q_index, ϵ_index, prev_ex_index, FC_0_index, FC_1_index] = 1
                                    n_func[Q_index, ϵ_index, prev_ex_index, FC_0_index, FC_1_index] = n_opt
                                    k_func[Q_index, ϵ_index, prev_ex_index, FC_0_index, FC_1_index] = k_opt
                                    if total_Π > candidate_max
                                        candidate_max = total_Π
                                    end
                                else
                                    ex_func[Q_index, ϵ_index, prev_ex_index, FC_0_index, FC_1_index] = 0
                                    n_func[Q_index, ϵ_index, prev_ex_index, FC_0_index, FC_1_index] = n_prev
                                    k_func[Q_index, ϵ_index, prev_ex_index, FC_0_index, FC_1_index] = k_prev
                        ...
                        v_next[Q_index, ϵ_index, prev_ex_index, FC_0_index, FC_1_index] = candidate_max #update value function
                    end
                end
            end
        end
...
function V_iterate(prim::Primitives, res::Results; tol::Float64 = 1e-3, err::Float64 = 1000.0)
    ...
    while err>tol #begin iteration
        ...
        err = abs.(maximum(v_next.-res.val_func))/abs(v_next[prim.nQ, prim.nϵ, 1, 3, 3]) #reset error level
...
function data_sim_delta_nsims(prim::Primitives, res::Results)
    @unpack ex_func, n_func, k_func, n_prev_ex, ϵ, Q, FC_0, FC_1, σ_e, ρ_e, σ_FC_0, σ_FC_1, FC_0_grid, FC_1_grid = res #unpack value function
    @unpack Q_grid, ϵ_grid, n_periods, n_firms, ρ_q, σ_q, n_sims = prim #unpack primitives
    ...
    for k = 1:n_sims
        ...
        for i = 2:n_periods
            ...
            for j = 1:n_firms
                ...
                rand_FC_0 = FC_0 + rand(Normal(0,σ_FC_0))
                rand_FC_0_index = findmin(abs.(rand_FC_0 .- FC_0_grid))[2]
                
                rand_FC_1 = FC_1 + rand(Normal(0,σ_FC_1))
                rand_FC_1_index = findmin(abs.(rand_FC_1 .- FC_1_grid))[2]
                
                firms_export_decisions[i,j,k] = ex_func[Q_index, ϵ_index, floor(Int,firms_export_capital[i-1,j,k]), rand_FC_0_index, rand_FC_1_index]
                ...
                firms_labor_decisions[i,j,k] = n_func[Q_index, ϵ_index, floor(Int,firms_export_capital[i-1,j,k]), rand_FC_0_index, rand_FC_1_index]
                firms_capital_decisions[i,j,k] = k_func[Q_index, ϵ_index, floor(Int,firms_export_capital[i-1,j,k]), rand_FC_0_index, rand_FC_1_index]               
...
function MSM_delta_func_first3(x)
    ...
    res.σ_FC_0 = x[1] # Random variation in FC_0
    res.FC_0_min = res.FC_0-3*res.σ_FC_0
    res.FC_0_max = res.FC_0+3*res.σ_FC_0
    res.nFC_0 = 5
    res.FC_0_grid = collect(range(res.FC_0_min, length = res.nFC_0, stop = res.FC_0_max))
    # Tauchen for FC_0, no persistence, IID around FC_1 w/σ_FC_1
    tauchen_res_FC_0 = tauchen(res.nFC_0, 0.0, res.σ_FC_0, res.FC_0)
    res.tauchen_trans_FC_0 = tauchen_res_FC_0.p
    ...
    res.σ_FC_1 = x[2] # Random variation in FC_1
    res.FC_1_min = res.FC_1-3*res.σ_FC_1
    res.FC_1_max = res.FC_1+3*res.σ_FC_1
    res.nFC_1 = 5
    res.FC_1_grid = collect(range(res.FC_1_min, length = res.nFC_1, stop = res.FC_1_max))
    # Tauchen for FC_1, no persistence, IID around FC_1 w/σ_FC_1
    tauchen_res_FC_1 = tauchen(res.nFC_1, 0.0, res.σ_FC_1, res.FC_1)
    res.tauchen_trans_FC_1 = tauchen_res_FC_1.p
    ...
    res.n_func = Array{Float64}(zeros(prim.nQ, prim.nϵ, res.n_prev_ex, res.nFC_0, res.nFC_1)) # initial worker function guess
    res.k_func = Array{Float64}(zeros(prim.nQ, prim.nϵ, res.n_prev_ex, res.nFC_0, res.nFC_1)) # initial capital function guess
    res.ex_func = Array{Float64}(zeros(prim.nQ, prim.nϵ, res.n_prev_ex, res.nFC_0, res.nFC_1)) # initial export function guess
    res.val_func = Array{Float64}(zeros(prim.nQ, prim.nϵ, res.n_prev_ex, res.nFC_0, res.nFC_1)) # initial value function guess
